...
public class CommonController {
	...
	// bno로 상세페이지 부르기 ---이 주석의 오른쪽 설명란은 볼 필요 없음. board/read/*란 주소 board/read슬래쉬 뒤에
	// 붙는 애들은 이녀석 적용이란 의미 -> httpServletRequest request는 clinet가 주소창에 입력한 요청을 담은 객체로
	// request.getRequestURI는 클라이언트가 친 주소창이고, 그걸 잘라서
	// http://localhost:8181/board/read/1의 bno인 1만 따로 bno라는 변수에 저장하고, vo에 bno=1담아서
	// jsp에 어트리뷰트 속성으로 보내서 jsp는 그 데이터로 클라이언트에게 보여줌/////////////////////
	...
	public String read(Model model, HttpServletRequest request) {
		...
		if (vo != null) { // client가 주소창에 board/read/3해서 bno가 3짜리인 boardVO를 부르면 bno가 3짜리인 vo 데이터를 받는데 만약
							// 삭제를해서 없다면 데이터가 null인 상세페이지 말고, list 페이지로 보낸다.
	...
	// 싫어요+1 ////////////////////////////////board/read에서 싫어요 버튼 누르면 ajax로 url:
	// "/read/plusUnlike/" + bno, 경로에 get 타입으로 요청하면 밑 GetMapping이 적용되서
	// service.unlikeBoard(bno)가 실행되어 해당 bno의 싫어요 1증가 후 싫어요 개수를 int로 result에
	// 반환(컨트롤러는 할거다한거고) read.jsp가 그 result를 받아서 비동기적으로 싫어요 갯수를 리로딩.
	...
	public String join(MemberVO vo, BindingResult result, RedirectAttributes rttr) {
		...
		boolean isDuplicated = service.isUniqueID(vo.getUserid()) && service.isUniqueNickName(vo.getNickname());
	...
	// 유저 닉네임 중복됬는지 체크
	@GetMapping(value = "/checkNickName/{nickName}", produces = { MediaType.TEXT_PLAIN_VALUE })
	@ResponseBody
